...
import 'package:aimsdigital/Models/Home/CoursesHome.dart';

import '../../../Bloc/Admin/AdminProductBloc/admin_course_bloc.dart';
...
class _AddAdminCoursestate extends State<AddAdminCourse> {
  ...
  void initState() {
    ...
    // Course.categoryId=
    // widget.Course.categoryId;
    // Course.codeCourse=
    // widget.Course.codeCourse;
    // procuct.description=
    ...
    return BlocListener<AdminCourseBloc, AdminCoursestate>(
      listener: (context, state) {
        if (state is LoadingAddCoursestate) {
          modalLoading(context, 'Adding Course...');
        } else if (state is AddCoursesuccessState) {
          Navigator.of(context).pop();
          modalFrave(context, 'Course Added Successfully');
        } else if (state is FailureAddCoursestate) {
          Navigator.of(context).pop();
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: CustomText(text: 'Error adding Course'),
              backgroundColor: Colors.red));
        } else if (state is LoadingUpdateCoursestate) {
          modalLoading(context, 'Updating Course...');
        } else if (state is UpdateCoursesuccessState) {
          Navigator.of(context).pop();
          modalFrave(context, 'Course Updated Successfully');
        } else if (state is FailureUpdateCoursestate) {
          Navigator.of(context).pop();
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: CustomText(text: 'Error Updating Course'),
              backgroundColor: Colors.red));
        }
      },
      child: Scaffold(
        ...
        appBar: AppBar(
            ...
            elevation: 0,
            title: CustomText(
                text: isUpdate ? "Update ${course.nameCourse}" : 'Add Course',
                color: Colors.black),
            centerTitle: true,
            leading: IconButton(
              icon: Icon(
        ...
        body: Form(
          ...
          child: ListView(
            ...
            children: [
              ...
              CustomText(text: 'Course Information', fontSize: 18),
              ...
              TextFormField(
                decoration:
                    InputDecoration(hintText: "Enter Course Description"),
              ...
              SizedBox(height: 20.0),
              TextFormField(
                decoration: InputDecoration(hintText: "Enter Course Code"),
