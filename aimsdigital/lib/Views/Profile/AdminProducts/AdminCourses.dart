  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: CustomText(text: 'Manage Courses', color: Colors.black),
        centerTitle: true,
        elevation: 0,
        leading: IconButton(
    ...
    return BlocListener<AdminCourseBloc, AdminCoursestate>(
      listener: (context, state) {
        if (state is DeleteCourseLoadingState) {
          modalLoading(context, 'Deleting Course...');
        } else if (state is DeleteCoursesuccessState) {
          setState(() {});
          Navigator.of(context).pop();
          modalFrave(context, 'Course Deleted Successfully');
        } else if (state is DeleteCourseFailureState) {
          Navigator.of(context).pop();
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: CustomText(text: 'Error Deleting Course'),
      ...
      child: Container(
        ...
        child: FutureBuilder<List<Course>>(
          ...
          builder: (context, snapshot) {
            print("This is snapsohot of Course data ${snapshot.data}");
